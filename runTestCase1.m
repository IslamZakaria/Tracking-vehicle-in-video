function runTestCase1( VideoName )
...
foregroundDetector = vision.ForegroundDetector('NumGaussians', 5, ...
            'NumTrainingFrames', 50,'MinimumBackgroundRatio',0.65);
...
while ~isDone(videoReader)
    ...
    if boxI>=1  %here i'm going to do some threshold. first for Width&Height of objects on bounding box then on there sizes 
        for i = 1:boxI
            if (bbox(i,3)>(9*videoWidth)/10 ) || ( bbox(i,3)<(1*videoWidth/10) ) || ( bbox(i,4)>(9*videoHeight)/10 ) || ( bbox(i,4)<(1*videoHeight)/10 ) || (bbox(i,1))+bbox(i,3)>videoWidth 
                bbox(i,:)=[0,0,0,0];
            else
