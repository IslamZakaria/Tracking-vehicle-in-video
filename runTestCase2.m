function runTestCase2( VideoName )
...
foregroundDetector = vision.ForegroundDetector('NumGaussians', 5, ...
            'NumTrainingFrames', 130,'MinimumBackgroundRatio',0.63);
...
while ~isDone(videoReader)
    ...
    if boxI>=1%here i'm going to do some threshold. first for Width&Height of objects on bounding box then on there sizes 
        for i = 1:boxI
            if ( bbox(i,3)>(3*videoWidth)/4 ) || ( bbox(i,3)<(3*videoWidth/20) ) || ( bbox(i,4)>(3*videoHeight)/4 ) || ( bbox(i,4)<(2*videoHeight)/20 ) || bbox(i,2)- bbox(i,4)/3 < 0
                bbox(i,:)=[0,0,0,0];
            else
                sizeBBox=bbox(i,3)*bbox(i,4);
                if sizeBBox<=3500 || sizeBBox>=270000
                    bbox(i,:)=[0,0,0,0];
                    savePosSingle=[];
                end
